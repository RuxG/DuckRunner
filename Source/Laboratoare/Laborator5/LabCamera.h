	...
	class Camera
	{
		public:
			...
			void MoveForward(float distance)
			{
				...
				this->position += distance * dir;
			}

			void TranslateForward(float distance)
			{
				// TODO : Translate the camera using the "forward" vector
				this->position += distance * glm::normalize(glm::vec3(forward.x, 0, forward.z));
			...
			void TranslateRight(float distance)
			{
				...
				this->position += distance * glm::normalize(glm::vec3(right.x, 0, right.z));
			}
			...
			void RotateFirstPerson_OX(float angle)
			{
				...
				// Use glm::rotate()
				forward = glm::normalize(glm::vec3(glm::rotate(glm :: mat4(1), angle, glm::vec3(1, 0, 0)) * glm::vec4(forward, 1)));
